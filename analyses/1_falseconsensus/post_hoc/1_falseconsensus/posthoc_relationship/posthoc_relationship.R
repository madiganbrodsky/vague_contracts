...
# load core dataset
d <- read_csv("/Users/madiganbrodsky/Desktop/legal_interpretation/vague_contracts/data/1_falseconsensus/main-merged.csv")
...
#PLOT #3: PROP YES / ERROR FOR CONTROVERSIAL STIMS 


new2 <- transformed %>% 
  filter(version != 'controversial', 
    individual_judgment != 'cantdecide')

ggplot(data = new2, 
       mapping = aes(x = individual_judgment, 
                     y = population_judgment)) + 
  geom_jitter(aes(color = individual_judgment), 
              alpha = 0.3)+ 
